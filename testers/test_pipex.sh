...
commands=(
    ...
    "./pipex infile.txt 'script.sh' 'wc' /dev/stdout"
    "time ./pipex infile.txt 'sleep 3' 'sleep 2' /dev/stdout"
    "./pipex Makefile 'echo 'bonjour'' 'tr 'a-z' 'A-Z'' /dev/stdout"
	"./pipex_bonus infile.txt cat cat cat /dev/stdout"
	"./pipex_bonus infile.txt cat 'grep pattern' sort uniq /dev/stdout"
	"./pipex_bonus infile.txt cat 'grep pattern' 'wc -l' sort /dev/stdout"
	"./pipex_bonus infile.txt cat 'tr a-z A-Z' 'grep pattern' 'wc -l' /dev/stdout"
	"./pipex_bonus infile.txt cat 'sed s/pattern/replacement/' sort uniq /dev/stdout"
	"./pipex_bonus infile.txt cat 'grep pattern' 'tr a-z A-Z' 'wc -l' /dev/stdout"
...
true_pipe=(
    ...
    "<infile.txt ./script.sh | wc> /dev/stdout"
    "time <infile.txt sleep 3 | sleep 2> /dev/stdout"
    "<Makefile echo 'bonjour' | tr 'a-z' 'A-Z'> /dev/stdout"
	"<infile.txt cat | cat | cat> /dev/stdout"
	"<infile.txt cat | grep 'pattern' | sort | uniq> /dev/stdout"
	"<infile.txt cat | grep 'pattern' | wc -l | sort> /dev/stdout"
	"<infile.txt cat | tr 'a-z' 'A-Z' | grep 'pattern' | wc -l> /dev/stdout"
	"<infile.txt cat | sed 's/pattern/replacement/' | sort | uniq> /dev/stdout"
	"<infile.txt cat | grep 'pattern' | tr 'a-z' 'A-Z' | wc -l> /dev/stdout"
