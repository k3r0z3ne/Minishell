...
commands=(
	"./pipex_bonus infile.txt cat \"tr ' ' ' '\" /dev/stdout"
    ...
    "./pipex_bonus infile.txt 'cat' 'tr -s \" \"' 'sort' 'uniq -c' 'sort -nr' /dev/stdout"
...
true_pipe=(
    ...
    "<infile.txt cat | tr -s \" \" | sort | uniq -c | sort -nr > /dev/stdout"
